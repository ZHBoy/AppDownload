...
import appupdate.zhouh.com.appdownload.appupdate.Constant;
import appupdate.zhouh.com.appdownload.appupdate.DownloadService;
...
public class AppUpdateUtil {
    ...
    public static void installApk(Context context, Uri apkPath) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri contentUri = null;
            try {
                contentUri = FileProvider.getUriForFile(context, "appupdate.zhouh.com.appdownload.fileProvider", new File(new URI(apkPath.toString())));
    ...
    public static void removeOldApk(Context context) {
        //获取老ＡＰＫ的存储路径
        File fileName = new File(SharedPreferencesUtils.get(context, Constant.INSTANCE.getSP_DOWNLOAD_PATH(), ""));
        Log.i("zhou", "老APK的存储路径 =" + SharedPreferencesUtils.get(context, Constant.INSTANCE.getSP_DOWNLOAD_PATH(), ""));

        if (fileName != null && fileName.exists() && fileName.isFile()) {
            fileName.delete();
            SharedPreferencesUtils.put(context, Constant.INSTANCE.getSP_DOWNLOAD_PATH(), ".");
